...
class TimeInterval(Base):
    ...
    interval_start = Column(TIMESTAMP)
...
class VariationMeasureValue(Base):
    ...
    # Many2One
    activity_id = Column(Integer, ForeignKey('activity.id')) # Todo implement
    user_in_role_id = Column(Integer, ForeignKey('user_in_role.id'))
    measure_type_id = Column(Integer, ForeignKey('cd_detection_variable.id'))
